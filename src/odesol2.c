...
int bak_euler(double *y, double *tim, double dt, int nt, int neq, int *istart, double *work) {
	...
	for(i=0;i<nt;i++) {
		if((j=one_flag_step_backeul(y,tim,dt,neq,yg,yp,yp2, ytemp,errvec,jac,istart))!=0) {
			return(j);
		}
		stor_delay(y);
	}
...
int one_bak_step(double *y, double *t, double dt, int neq, double *yg, double *yp, double *yp2,
