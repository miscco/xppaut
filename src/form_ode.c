...
static int do_new_parser(FILE *fp, char *first, int nnn) {
	...
	/*int nlin;
 */
	char big[MAXEXPLEN],old[MAXEXPLEN],new_char[MAXEXPLEN];
	char *my_string;
	int is_array=0;
	if(nnn==0) {
		init_varinfo();
	...
	while(notdone) {
		...
		search_array(old,new_char,&jj1,&jj2,&is_array);
		jj=jj1;
		jjsgn=1;
		if(jj2<jj1) {
			jjsgn=-1;
		...
		case 1:
			nstrings=1;
			strings[0]=(char *)malloc(strlen(new_char)+10);
			strcpy(strings[0],new_char);
			break;
		...
		while(1) {
			for(ns=0;ns<nstrings;ns++) {
				strcpy(new_char,strings[ns]);
				subsk(new_char,big,jj,is_array);
 ...
 */
	...
	/* now we add all the names of the variables and the fixed stuff */
	for(i=0;i<nvar;i++) {
